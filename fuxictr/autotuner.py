...
import sys
sys.path.append("/rqz/public/rqz/code/")
import itertools
import subprocess
import yaml
import os
...
def enumerate_params(config_file, exclude_expid=[]):
    ...
    # enumerate src para combinations
    ...
    # dump src para combinations to config file
...
def grid_search(config_dir, gpu_list, expid_tag=None, script='run_expid.py'):
    ...
    while len(experiment_id_list) > 0:
        if len(idle_queue) > 0:
            ...
            # cmd = "/home/lhw/anaconda3/envs/FuxiCTR/bin/python -u {} --config {} --expid {} --gpu {}"\
            #         .format(script, config_dir, expid, gpu_id)
            # cmd = "/root/miniconda3/bin/python -u {} --config {} --expid {} --gpu {}" \
            #     .format(script, config_dir, expid, gpu_id)
            # cmd = "/root/miniconda3/envs/fuxiCTR/bin/python -u {} --config {} --expid {} --gpu {}" \
            #     .format(script, config_dir, expid, gpu_id)
            cmd = "/home/rqz/miniconda3/envs/CTR/bin/python -u {} --config {} --expid {} --gpu {}"\
                     .format(script, config_dir, expid, gpu_id)
